...
function FixedAssetsContent() {
  ...
  const isEmbedded = searchParams.get("embedded") === "1" || pathname === "/assets";
  ...
  const { data: ledgerBalancesData, refetch: refetchLedgerBalances } = useLedgerBalances("Fixed Assets");
  ...
  const fixedAssetsParent = parentGroups.find(
    (pg) => pg.name === "Fixed Assets"
  );

  // Filter ledger groups that belong to "Fixed Assets"
  ...
  const fixedAssetLedgers = ledgers.filter((ledger) =>
    fixedAssetGroups.some((group) => group.id === ledger.ledger_group_id)
  );
          ...
          {assetsByLedger.length > 0 ? (
            ...
            <div className="mb-8 rounded-xl border border-zinc-200 bg-white p-8 text-center shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
              <p className="text-zinc-600 dark:text-zinc-400">
                No fixed assets found. Create asset accounts in{" "}
                <a
                  href="/accounts"
...
const fixedAssetsSuspenseFallback = (
