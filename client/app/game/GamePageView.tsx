...
import { Suspense, useState, useMemo, useEffect, useRef, memo } from "react";
...
import { getGameSave, putGameSave } from "@/lib/api/game-save";
...
/** Reported to a parent page (e.g. the Investments page's "Investment Game" tab) so its own Header can show available resources, instead of GamePage rendering a duplicate copy in its own body. */
...
/** Normalizes a raw API `state` payload (possibly `{}` for "no save yet") into a `GameState`. */
function parseGameStatePayload(raw: Record<string, unknown>): GameState {
  ...
  return {
    ...
    hoursAvailable: state.hoursAvailable ?? 300,
  };
}
...
function GamePageContent({
  ...
  onHeaderInfoChange?: (info: GameHeaderInfo | null) => void;
}) {
  ...
  // Game state now persists server-side, per user (see GET/PUT /investments/game-save).
  ...
  const [gameSyncError, setGameSyncError] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const { state: raw } = await getGameSave(token);
        if (cancelled) return;
        setGameState(parseGameStatePayload(raw));
      } catch {
        if (cancelled) return;
        setGameState(createDefaultGameState());
        setGameSyncError("Could not load your saved game.");
      } finally {
        if (!cancelled) {
  ...
  useEffect(() => {
    if (gameSyncError == null) return;
    const id = window.setTimeout(() => setGameSyncError(null), 4000);
  ...
  // Auto-save game state to the server (debounced), per user.
  useEffect(() => {
    if (!token || !gameLoadComplete) return;
    ...
    const timeoutId = setTimeout(() => {
      void putGameSave(token, gameState).catch(() => {
        setGameSyncError("Could not save your game.");
      });
    }, 1000); // Debounce: save 1 second after last change

    return () => clearTimeout(timeoutId);
  }, [gameState, token, gameLoadComplete]);
  ...
  const handleReset = async () => {
    ...
    if (confirmed) {
      ...
      // Reset overwrites the server-side save immediately (the debounced
      // auto-save effect intentionally skips "unstarted" states, so it
      // wouldn't otherwise persist this reset).
      skipNextPersistRef.current = true;
      if (token) {
        void putGameSave(token, freshState).catch(() => {
          setGameSyncError("Could not save the reset game.");
        });
      }
  ...
  // Clear reported header info on unmount (e.g. when the parent switches away from the game tab).
  useEffect(() => {
    return () => onHeaderInfoChange?.(null);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  useEffect(() => {
    if (!onHeaderInfoChange) return;
    onHeaderInfoChange({
      availableCash: gameState.currentMoney,
      hoursAvailable: gameState.hoursAvailable ?? 300,
    });
  }, [onHeaderInfoChange, gameState.currentMoney, gameState.hoursAvailable]);
    ...
    <div className="flex min-h-screen flex-col bg-zinc-50 dark:bg-zinc-950" suppressHydrationWarning>
      ...
      >
        <div className="mx-auto w-full max-w-[98%] px-2 py-3 md:px-3 md:py-4 lg:px-4 lg:py-5">
              ...
              <button
                onClick={async () => {
                  ...
                  try {
                    ...
                    skipNextPersistRef.current = true;
                    setGameState(parseGameStatePayload(raw));
                    alert("Game data refreshed from the server.", "Refresh Successful");
                  } catch {
                    alert("Could not load your saved game from the server.", "Refresh Failed");
                  }
                }}
          ...
          {gameSyncError && (
            <p
              className="mb-4 rounded-lg border border-rose-200 bg-rose-50 px-3 py-2 text-sm text-rose-900 dark:border-rose-900 dark:bg-rose-950/50 dark:text-rose-100"
              role="alert"
            >
              {gameSyncError}
            </p>
          ...
          {!gameState.gameOver && (
            <>
              <div className="mb-4 flex flex-wrap gap-4">
                ...
                <div className="min-w-[200px] flex-[0.7]">
                  <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-2 shadow-sm dark:border-emerald-800 dark:bg-emerald-900/20">
                ...
                <div className="min-w-[200px] flex-1">
                  <div className="rounded-xl border border-orange-200 bg-orange-50 p-2 shadow-sm dark:border-orange-800 dark:bg-orange-900/20">
                    <div className="mb-2 flex items-center justify-between">
  ...
  return (
    <DialogProvider>
      <Suspense fallback={gamePageSuspenseFallback}>
        <GamePageContent onHeaderInfoChange={onHeaderInfoChange} />
      </Suspense>
    </DialogProvider>
  );
});
